...
If the input is an OWID-style dataset or the task gives only a chart name plus CSV, also read [references/owid-rules.md](references/owid-rules.md). OWID samples are high-risk because the agent must infer the chart brief.
...
| OWID inference rules | [owid-rules.md](references/owid-rules.md) | Chart-name-only or OWID-style CSV tasks |
